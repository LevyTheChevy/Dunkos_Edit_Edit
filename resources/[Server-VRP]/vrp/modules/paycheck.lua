...
AddEventHandler('paycheck:salary', function()
	...
	if vRP.hasPermission(user_id,"Captain.paycheck") then
		...
		if vRP.hasPermission(user_id,"off.paycheck") then
		vRP.giveMoney(user_id,500)
		vRPclient.notify(source,{"Bonus $500"})
	end
...
RegisterServerEvent('paycheckbonus')
AddEventHandler('paycheckbonus', function()
  	local user_id = vRP.getUserId(source)
	if vRP.hasPermission(user_id,"user.paycheck") then
		vRP.giveMoney(user_id,100)
		vRPclient.notify(source,{"Tax Return! $1000"})
	end
	if vRP.hasPermission(user_id,"user.paycheck") then
		vRP.getMoney(user_id,-500)
		vRPclient.notify(source,{"IRS $-500"})
	end
end)